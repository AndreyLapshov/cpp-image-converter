...
bool SaveBMP(const Path& file, const Image& image) {
    ...
    file_header.bfSize = image.GetHeight() * stride + file_header.bfOffBits;
    info_header.biWidth = image.GetWidth();
    info_header.biHeight = image.GetHeight();
    info_header.biSizeImage = image.GetHeight() * stride;
...
Image LoadBMP(const Path& file) {
    ...
    if (file_header.bfType[0] != 'B' || file_header.bfType[1] != 'M' ||
        info_header.biXPelsPerMeter != 11811 || info_header.biYPelsPerMeter != 11811) {
        return {};
    }
